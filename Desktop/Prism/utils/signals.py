def extract_signals(agent_outputs: dict) -> list:
    ...
    # ── Agent 4 — Career Page Validator ──────────────────
    if a4.get("confidence", 0) > 0.4:
        if a4.get("careers_page_found") and not a4.get("role_found_on_careers_page"):
            signals.append({
                "signal": "role_not_on_careers_page",
                "severity": "medium",
                "detail": "Role not listed on company's own careers page — could mean filled, removed, or page not updated",
                "agent": "career_page_validator"
            })
